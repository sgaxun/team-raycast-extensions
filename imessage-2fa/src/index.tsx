...
export default function Command() {
    ...
    if (data) {
        const code : string = extractCode(data[0].text)!;
        showToast(Toast.Style.Success, "Copied", code + "已填充").then(() => console.error({code}))
        exec(`echo copied`);

        Clipboard.paste(code).then(r => console.log(r));
    }
...
function SearchTypeDropdown(props: { onChange: (searchType: SearchType) => void }) {
    return (
        <List.Dropdown tooltip="Filter search type" storeValue
        ...
        <List.Item.Detail
            ...
            metadata={
                <List.Item.Detail.Metadata>
                    ...
                    <List.Item.Detail.Metadata.Label title="Date"
                                                     text={new Date(props.message.message_date).toLocaleString()}/>
                </List.Item.Detail.Metadata>
            }
        />
    );
